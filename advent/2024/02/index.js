...
// traverse through array serialize into 2 arrays: left column / right column
for (let i = 0; i < columns.length; i++) {
  const numbers = columns[i].replace(/\s+/g, ",").split(",");
  leftColumn.push(numbers[0]);
  rightColumn.push(numbers[1]);
}

let sum = 0;

// traverse through first column
for (let i = 0; i < leftColumn.length; i++) {
  let similarityScore = 0;
  for (let j = 0; j < rightColumn.length; j++) {
    // on each iteration identify the number of times that number appears on right
    // column
    if (leftColumn[i] === rightColumn[j]) {
      similarityScore++;
    }
  }
  sum += leftColumn[i] * similarityScore;
}

console.log(sum);

// idea: create a key value for identified numbers to skip reoccurring instances
// get the sum of all iterations products
