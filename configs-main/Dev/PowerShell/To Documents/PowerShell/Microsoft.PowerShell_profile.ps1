...
# Start oh-my-posh
oh-my-posh init pwsh --config "C:\Users\Eduardo\AppData\Local\Programs\oh-my-posh\themes\edu_tokyostorm.omp.json" | Invoke-Expression
oh-my-posh init pwsh | Invoke-Expression

function rf { refreshenv; Write-Output 'Refreshed environment variables you scumbag!'; Start-Sleep 1; Clear-Host; }
function cl { Clear-Host; }

function Start-MySQL {
  $mysqlServiceName = 'MySQL81' # You might need to adjust this to the correct service name
  $mysqlService = Get-Service -Name $mysqlServiceName -ErrorAction SilentlyContinue

  if (-not $mysqlService) {
    Write-Warning "MySQL service ($mysqlServiceName) not found."
    return
  }

  if ($mysqlService.Status -ne 'Running') {
    Write-Output "Startin;g MySQL service..."
    Start-Service -Name $mysqlServiceName
    if ((Get-Service -Name $mysqlServiceName).Status -eq 'Running') {
      Write-Output "MySQL service started successfully."
    }
    else {
      Write-Warning "Failed to start MySQL service."
    }
  }
  else {
...
Set-Alias -Name startmysql -Value Start-MySQL
